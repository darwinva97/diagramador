...
- **Zoom del diagrama** del 25 % al 200 %, independiente del resto de la interfaz: controles abajo a la izquierda, Ctrl + rueda (centrado en el puntero), Ctrl +, Ctrl − y Ctrl 0, más un botón para ajustar el ancho a la ventana. Por encima del 100 % las cabeceras fijas se sueltan solas: Chrome calcula mal qué hay bajo el cursor cuando combina `zoom` con `position: sticky` y el arrastre dejaría de ser preciso.
