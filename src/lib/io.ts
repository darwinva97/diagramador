...
export function exportAll(data: AppData) {
  const f: ExportFile = { app: 'diagramador', version: 1, exportedAt: new Date().toISOString(), libraries: data.libraries, diagrams: data.diagrams };
...
export function mergeImport(data: AppData, raw: unknown): string | null {
  ...
  const inc = normalize({ libraries: obj.libraries ?? [], diagrams: obj.diagrams ?? (obj.diagram ? [obj.diagram] : []) });
