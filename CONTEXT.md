...
- `src/` — React 19 + TypeScript + Vite + React Router (declarativo, `routes.tsx`: `/d/:id`, `/cuenta/:tab`, `/ventana/:view`), estado con Zustand (immer + persist en `localStorage`, clave `diagramador.v2`).
  ...
  - `clipboard.ts` copiar/cortar/pegar instancias y componentes en la celda activa (`store.cell`) · `lib/schema.ts` reconocimiento de la estructura de un JSON y campos conectables de un componente.
