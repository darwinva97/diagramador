...
- **API (Api)**: catálogo aparte (`data.apis`, `kind: 'api'`, rutas `/apis`): nombre, repositorio, docs, versión, autenticación, `baseUrls[]` (entorno → URL) y `operations[]` (`ApiOperation`: método, ruta, parámetros, cabeceras, request/response JSON, códigos). Se define una vez y se usa en muchas celdas.
  - **Vínculo especial**: cada API tiene un **componente espejo** en la librería `lib-apis` (id fijo `api-{apiId}`), que mantiene `syncApiComponent` a partir de la API. Las instancias apuntan a ese componente, así que tablero, flechas, reglas y personas siguen funcionando sin cambios.
  - De cada instancia son sólo `placement.operationId` (qué operación usa ahí) y `placement.note`. Todo lo demás es de la API: editarla cambia todos sus usos. La interfaz lo impone (`ApiInstancePanel` no edita la API; el menú del chip tampoco) y `userLibs()` mantiene el catálogo fuera de las librerías de componentes.
  - `lib/api.ts` también convierte a y desde **OpenAPI** (`toOpenApi` / `fromOpenApi`).
...
- `src/` — React 19 + TypeScript + Vite + React Router (declarativo, `routes.tsx`: `/d/:id`, `/cuenta/:tab`, `/ventana/:view`), estado con Zustand (immer + persist en `localStorage`, clave `diagramador.v2`).
  ...
  - `PublicPanel.tsx` ficha de detalle de la vista pública: **no es el inspector apagado**, es un panel que sólo enseña (componente, relación o portada con la leyenda de reglas), para que no haya ningún control de edición que pueda confundir. En sólo lectura el tablero permite arrastrar una instancia **dentro de su celda de origen** (`dentroDeLaCelda` en `Board`, acota x e y al rectángulo de la celda y vuelve a llamar a `movePlacement` con su misma capa y etapa); los subcomponentes no se mueven, porque los coloca su contenedor. El cambio vive en el store del visitante (`drawer.public`) y se pierde al recargar.
  - `ApiPanel.tsx` dos paneles a propósito distintos: `ApiPanel` edita la API (y avisa de cuántos usos afecta) y `ApiInstancePanel` sólo elige operación y escribe la nota de esa celda.
  - `Share.tsx` diálogo de compartir (`openShare(id)`), al que llevan la barra superior, la lista de `/cuenta/diagramas` y el inspector del diagrama.
